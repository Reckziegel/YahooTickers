...
The goal of YahooTickers is to help users to easily get stock index constituents from [Yahoo Finance](https://finance.yahoo.com/) without having to run complicated `for` loops. This may be specially useful for users that need to repeatably test investment strategies in different markets, but also convenient if data is needed for it's side effects (plotting).  
...
Since the data is "tidy" it's ready to be used in conjunction with `ggplot2` and the `dplyr` verbs for further manipulation.
